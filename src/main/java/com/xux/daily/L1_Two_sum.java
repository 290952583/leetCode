...
public class L1_Two_sum {
     ...
     * 复杂度分析：
     * 时间复杂度：O(n^2)， 对于每个元素，我们通过遍历数组的其余部分来寻找他对应的目标元素，这将消耗O(n),因此时间复杂度为O(n^2)
     * 空间复杂度：
     * O(1)
     * @param nums
     * @param target
    ...
    public static int[] twoSum2(int[] nums, int target) {
        ...
        for (int i = 0; i < nums.length; i++) {
            int template = target - nums[i];
            if (map.containsKey(template) && map.get(template) != i){
                return new int[]{i, map.get(template)};
            }
        }

        throw new IllegalArgumentException("No two sum L1_Two_sum");
    }

    /**
     * 解法三：一遍哈希表
     * 事实证明，我们可以一次完成。在进行迭代并将元素插入到表中的同时，
     * 我们还会回过头来检查表中是否已经存在当前元素所对应的目标元素。如果它存在，那我们已经找到了对应解，并立即将其返回
     * @param nums
     * @param target
     * @return
     */
    public static int[] twoSum3(int[] nums, int target) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (map.get(template) != null){
                return new int[]{map.get(template), i};
            }
            map.put(nums[i], i);
        }
        throw new IllegalArgumentException("No two sum L1_Two_sum");
